...
#include "sdpa_x86_avx.h"

#include "layer_type.h"

namespace ncnn {

SDPA_x86_avx::SDPA_x86_avx()
{
    qk_gemm = 0;
    qkv_gemm = 0;
    qk_softmax = 0;
...
int SDPA_x86_avx::create_pipeline(const Option& _opt)
{
    Option opt = _opt;
    if (int8_scale_term)
    {
...
int SDPA_x86_avx::destroy_pipeline(const Option& _opt)
{
    Option opt = _opt;
    if (int8_scale_term)
    {
...
int SDPA_x86_avx::forward(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& _opt) const
{
    Option opt = _opt;
    if (int8_scale_term)
    {
